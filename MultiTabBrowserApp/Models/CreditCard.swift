...
class CreditCard {
    ...
    class func regularExpression(for cardType: CardType) -> String {
        switch cardType {
        case .amex:
            return "^3[47][0-9]{5,}$"
        case .visa:
    ...
    class func suggestionRegularExpression(for cardType: CardType) -> String {
        switch cardType {
        case .amex:
            return "^3[47][0-9]+$"
        case .visa:
    ...
    class func isValidEditable(with cardNumber: String) -> Bool {
        let formattedCardNumber = cardNumber.replacingOccurrences(of: "[^0-9]", with: "", options: .regularExpression)
        let count = formattedCardNumber.count
        do {
            let type = try cardType(for: formattedCardNumber)
    ...
    class func performAlgorithm(with cardNumber: String) throws {
        ...
        do {
            ...
            case .amex:
                ...
                break
            case .visa:
                guard count == 13 || count == 16 || count == 19 else {
                    throw CardError.invalid
                }
                break
            case .mastercard:
